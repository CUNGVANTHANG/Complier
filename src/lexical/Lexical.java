...
import static shared.ErrorHandler.error;
...
public class Lexical {
    ...
    Token commentDetector(int line, int pos) {
        ...
        if (firstChar == '*') {
            ...
            while (true) {
                if (this.chr == '\u0000') {
                    error(line, pos,  String.format("Unexpected end of file in multi-line comment.\nYou might be missing the '*/' to close the comment. %s", this.chr));
            ...
            error(line, pos, String.format("Unexpected token: %s", this.chr));
            return null;
        }
    }
